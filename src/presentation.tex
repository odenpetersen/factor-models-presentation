...
\begin{frame}{Spot Transactions}
	...
	If I give $q>0$ units of some asset $A$, and you give me $\$pq$, then:%I'm gonna use dollar signs here although often we trade in many currencies. It is typical to emphasise one currency which we call the numeraire and express things in terms of it.
	\begin{itemize}
...
\begin{frame}{Dutch Book Theorems}
	...
	%Exercise: extend this reasoning to deal with spreads
	%Exercise: justify Bayes rule

	The market for Arrow-Debreu securities is arbitrage-free if and only if $\mathbb{Q}_{t'}(\cdot)$ obeys the axioms of Kolmogorov (1950), i.e. is a \textbf{probability measure} on $\mathcal{F}_t$.%Kolmogorov (1950)
\end{frame}
...
\begin{frame}{Estimation}
	...
	where $D^+$ replaces the nonzero elements of $D$ with their inverse.%Double check that this works as described
...
\begin{frame}{Factor Models}
	...
	\end{align*}
	where $U_m$ consists of the first $m$ principal components of $\Sigma_\epsilon^{-\frac{1}{2}}(R-\overline{R})$.%Tipping and Bishop
	...
	If $B$ and $\Sigma_\epsilon^{-1}$ are known, an unbiased estimator for the factor covariance $\Sigma_f$ is given by $\frac{1}{n}F F^\top - (B\top \Sigma_\epsilon^{-1} B)^{-1}$.%In practice not too much uncertainty about idio vols
...
\begin{frame}{Market Efficiency}
	...
	However, most assets give their holder the right to potential future dividends. %Exercise: find a counterexample
...
\begin{frame}{Statistical Arbitrage}
	...
	\\	\hat{x}_i'	&= F\hat{x}_{i-1},	%&= [0\\0\\ \hat{\epsilon}_{i-1}]
	\\	P_i'		&= FP_{i-1}F^\top+Q,	%&= [\Sigma_f,0,0\\0,I,0\\0,0,P^\epsilon_{i-1}]
	\\	y		&= r_i - H \hat{x}_i',	%&= r_i - \Theta_1 \hat{\epsilon}_{i-1}
	\\	S		&= H P_i' H^\top,	%&= B\Sigma_f B^\top + \Theta_0\Theta_0^\top + \Theta_1 P^\epsilon_{i-1}\Theta_1^\top
	\\	K		&= P_i' H^\top S^{-1},  %= [\Sigma_f B^\top \\ \Theta_0 \\ P^\epsilon_{i-1} \Theta_1](B\Sigma_f B^\top + \Theta_0\Theta_0^\top + \Theta_1 P^\epsilon_{i-1}\Theta_1^\top)^{-1}
	\end{align*}
	we then have
	\begin{align*}
	...
	%L(B\Sigma_f B^\top + \Theta_0^2 + \Theta_1 P^\epsilon_i \Theta_1)^{-1} \Theta_1 P^\epsilon_i
	%Alpha vector \Theta_1 P^\epsilon_i. Is this orthogonal to B

	%Pairs trading & vol weighting heuristic for legs
...
\begin{frame}{Signal Aggregation}
	...
	%Exercise: extend to multiple assets, each with various forecasts

	The portfolio return for the $i$th time period will be $R_i^A = r_i^A x_i \cdot \beta$.

	%Move this to the generic portfolio optim section
...
\begin{frame}{Transaction Cost Analysis}
	...
	and the optimal $\beta$ is given by
	$$\beta = L(\frac{2L\pi^2}{n} dX^\top \Lambda dX + \frac{1}{n}F^\top F)^{-1}\overline{F}.$$
...
\begin{frame}{Stuff I missed}
	...
	%Computational aspects
		...
		%Performance research
			%Quantifying latency eg clock drift
\end{frame}
