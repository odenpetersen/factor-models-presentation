...
\begin{frame}{Limit Order Book}%Special kind of data structure
	At any point in time, market participants can create a request (`\textbf{limit order}') of the form $(s,q,p)$ to trade up to $q$ units in direction $s=\pm1$ at a price $\$p$ (or better). %Explain 'better'. $p is sometimes called the reserve price
	...
	Users can add, cancel and modify orders at any time.%Limits on modification. Ratelimits, message limits etc.
	...
	If these orders are \textbf{matched}, $q$ units will trade at $\$p$ and the orders will become $(+1,p_1,q_1-q), (-1,p_2,q_2-q)$. %A match is also called a fill
...
\begin{frame}{Market Data \& Market Prices}
	...
	% Theoretical value
		% Efficiency
			%Edge
				%Latency (is this immoral somehow)
				%PFOF
				%Good pricing
...
\begin{frame}{Stuff I missed}
	...
	% How to find alpha
	% How to get a job
	% Adverse selection. "Certainly, the modern compendium of mental illnesses (DSM-5) takes a dim view of people who think everyone is out to get them. Yet financial markets are different: people really are out to get you, after all." - Agustin Lebron, The Laws of Trading
